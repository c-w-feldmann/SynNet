...
def test_shingling_from_mol(mol):
    shingling, atom_indices = DrfpEncoder.shingling_from_mol(mol)
...
def test_internal_encode(rxn, rxn_with_reagent):
    fp, ngrams, atom_indices = DrfpEncoder.internal_encode(rxn)
    fp_2, ngrams_2, atom_indices = DrfpEncoder.internal_encode(rxn_with_reagent)
